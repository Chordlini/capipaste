...
/// `eyes`: 1 open … 0 closed. `mouth`: 0 = closed smile, up to 1 = wide open.
/// `turn`: rotation around the vertical axis in radians (0 = facing you).
func drawAcorn(_ c: CGContext, eyes: CGFloat = 1, mouth: CGFloat = 0, turn: CGFloat = 0) {
    ...
    // Face features sit on the nut's surface; `turn` spins them around the vertical axis.
    ...
    for x: CGFloat in [410, 590] {
        place(x) {
            if eyes > 0.2 {
                let h = 70 * eyes
                c.setFillColor(gray: 0, alpha: 1)
                c.fillEllipse(in: CGRect(x: -30, y: 640 - h / 2, width: 60, height: h))
                if eyes > 0.7 {
                    c.setFillColor(gray: 1, alpha: 1)
                    c.fillEllipse(in: CGRect(x: -16, y: 616, width: 22, height: 22))
                }
            } else {
                // closed: a little downward arc
                c.setStrokeColor(gray: 0, alpha: 1)
                c.setLineWidth(16)
                c.setLineCap(.round)
                c.move(to: pt(-32, 640)); c.addQuadCurve(to: pt(32, 640), control: pt(0, 665))
                c.strokePath()
            }
        }
        let blush: CGFloat = x < 500 ? x - 42 : x + 42
        place(blush) {
            c.setFillColor(gray: 0.7, alpha: 1)
            c.fillEllipse(in: CGRect(x: -20, y: 690, width: 40, height: 24))
        }
    }
    place(500) {
        c.setStrokeColor(gray: 0, alpha: 1)
        ...
        c.setLineCap(.round)
        if mouth <= 0 {
            c.move(to: pt(-30, 698)); c.addQuadCurve(to: pt(30, 698), control: pt(0, 730))
            c.strokePath()
        } else {
            let (w, h) = (46 + 24 * mouth, 18 + 58 * mouth)
            c.setFillColor(gray: 0, alpha: 1)
    ...
    clipped(c, cap) {
        ...
        c.setLineWidth(11)
        // the scale lattice scrolls sideways as the acorn turns
        let shift = (turn * 235).truncatingRemainder(dividingBy: 84)
        for offset in stride(from: -600.0 - shift, through: 1200, by: 84) {
    ...
    // stem and leaf swing across as the acorn turns
    c.translateBy(x: 500, y: 0)
    c.scaleBy(x: cos(turn), y: 1)
    c.translateBy(x: -500, y: 0)
    c.setFillColor(gray: 0.05, alpha: 1)
...
case "icon":
    ...
    field.vignette()
    field.inBox(CGRect(x: 0, y: -1.1, width: CGFloat(grid), height: CGFloat(grid))) { drawAcorn($0) }
...
case "mark":
    // Just the acorn: ink dots on transparency, no paper, card or border dots.
    let px = args.count > 3 ? Int(args[3])! : 1024
    let grid = 112
    let field = Field(cols: grid, rows: grid)
    field.inBox(CGRect(x: 0, y: -2, width: CGFloat(grid), height: CGFloat(grid))) { drawAcorn($0) }
    // Crop to the inked dots, then fit them into the canvas with an 8% margin.
    let all = field.dither()
    let ys = all.indices.filter { all[$0].contains(true) }
    let xs = (0..<grid).filter { x in all.contains { $0[x] } }
    let dots = all[ys.first!...ys.last!].map { Array($0[xs.first!...xs.last!]) }
    let (w, h) = (CGFloat(dots[0].count), CGFloat(dots.count))
    let pitch = CGFloat(px) * 0.84 / max(w, h)
    let ctx = canvas(px, px)
    print(dots, into: ctx, rect: CGRect(x: (CGFloat(px) - w * pitch) / 2, y: (CGFloat(px) - h * pitch) / 2, width: w * pitch, height: h * pitch), pitch: pitch)
    save(ctx)

case "talk":
    // Talking, blinking acorn as a looping transparent GIF (12 fps, 4 s); `--spin` also turns it in place (15 fps, 4 s).
    let px = args.count > 3 ? Int(args[3])! : 512
    let spinning = args.contains("--spin")
    let grid = 112
    let fps = spinning ? 15 : 12
    let frameCount = spinning ? 60 : 48
    // mouth openness per frame: bursts of "syllables" with small pauses
    let syllables: [CGFloat] = [0, 0.35, 0.8, 0.4, 1, 0.55, 0.2, 0, 0, 0.6, 0.9, 0.3, 0.7, 0.45, 0, 0,
                                0.3, 1, 0.65, 0.85, 0.35, 0.15, 0, 0, 0.5, 0.75, 0.95, 0.4, 0.2, 0.6, 0.3, 0,
                                0, 0, 0.45, 0.8, 0.5, 1, 0.7, 0.3, 0.55, 0.25, 0, 0, 0, 0, 0, 0]
    let eyesAt: [Int: CGFloat] = [18: 0.5, 19: 0, 20: 0.5, 38: 0.5, 39: 0, 40: 0, 41: 0.5]
    let frames: [[[Bool]]] = (0..<frameCount).map { i in
        let field = Field(cols: grid, rows: grid)
        let turn = spinning ? CGFloat(i) / CGFloat(frameCount) * 2 * .pi : 0 // one full turn per loop
        field.inBox(CGRect(x: 0, y: -2, width: CGFloat(grid), height: CGFloat(grid))) {
            drawAcorn($0, eyes: eyesAt[i % 48] ?? 1, mouth: syllables[i % syllables.count], turn: turn)
        }
        return field.dither()
    }
    // one crop box for every frame so nothing jitters
    let ys = (0..<grid).filter { y in frames.contains { $0[y].contains(true) } }
    let xs = (0..<grid).filter { x in frames.contains { f in f.contains { $0[x] } } }
    let (w, h) = (CGFloat(xs.count), CGFloat(ys.count))
    let pitch = CGFloat(px) * 0.84 / max(w, h)
    let url = URL(fileURLWithPath: out) as CFURL
    let gif = CGImageDestinationCreateWithURL(url, "com.compuserve.gif" as CFString, frames.count, nil)!
    ...
    for frame in frames {
        let dots = frame[ys.first!...ys.last!].map { Array($0[xs.first!...xs.last!]) }
        let ctx = canvas(px, px)
        print(dots, into: ctx, rect: CGRect(x: (CGFloat(px) - w * pitch) / 2, y: (CGFloat(px) - h * pitch) / 2, width: w * pitch, height: h * pitch), pitch: pitch)
...
case "banner":
    ...
    field.inBox(CGRect(x: left, y: (CGFloat(rows) - art) / 2 - 1, width: art, height: art)) { drawAcorn($0) }
